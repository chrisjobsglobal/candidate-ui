...
import { UserSkillsStore } from '../store/user-skills.store';
    ...
    >
      ...
      >
        ...
        <div class="text-center mb-6">
          ...
          <p class="text-sm text-gray-600" *ngIf="skill">
            You are about to delete <span class="font-semibold">{{ skill.skill_name }}</span> 
            ({{ getStarsFromRating(skill.rating) }}/5 stars, Level {{ skill.rating }}) from your profile. This action cannot be undone.
          </p>
        </div>
...
export class DeleteSkillModalComponent {
  private readonly userSkillsStore = inject(UserSkillsStore);

  @Input() skill: UserSkill | null = null;
  @Output() close = new EventEmitter<void>();
  @Output() deleted = new EventEmitter<UserSkill>();
  ...
  onConfirmDelete(): void {
    if (!this.skill || this.isDeleting()) return;

    this.isDeleting.set(true);
    this.error.set(null);

    this.userSkillsStore.deleteItem(this.skill.id).subscribe({
      next: () => {
